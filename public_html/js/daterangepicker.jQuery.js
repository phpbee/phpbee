...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			{text: 'Week', dateStart: 'today-7days', dateEnd: 'today' },
			{text: 'Last 30 Days', dateStart: 'Today-30', dateEnd: 'Today' },
			{text: 'Month', dateStart: function(){ return Date.parse('today').moveToFirstDayOfMonth();  }, dateEnd: 'today' },
			{text: 'Prev month', dateStart: function(){ return Date.parse('1 month ago').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parse('1 month ago').moveToLastDayOfMonth();  } }
			//{text: 'Year', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			//extras:
	...
	jQuery.fn.clickActions = function(rp, rpPickers, doneBtn){
		...
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			rp.find('.range-start').restoreDateFromData().show(cl_timeout);
			rp.find('.range-end').restoreDateFromData().hide(cl_timeout);
			setTimeout(function(){doneBtn.fadeIn();}, cl_timeout);
		}
		else if(jQuery(this).is('.ui-daterangepicker-allDatesBefore')){
			doneBtn.hide();
			rpPickers.show();
			rp.find('.title-end').text( options.presets.allDatesBefore );
			rp.find('.range-start').saveDateToData().datepicker('setDate', options.earliestDate).hide(cl_timeout);
			rp.find('.range-end').restoreDateFromData().show(cl_timeout);
			setTimeout(function(){doneBtn.fadeIn();}, cl_timeout);
		}
			...
			rp.find('.range-start').restoreDateFromData().show(cl_timeout);
			rp.find('.range-end').saveDateToData().datepicker('setDate', options.latestDate).hide(cl_timeout);
			setTimeout(function(){doneBtn.fadeIn();}, cl_timeout);
		}
			...
			//custom date range
				doneBtn.hide();
				rp.find('.range-start, .range-end').hide(cl_timeout, function(){
					rpPickers.hide();
				});

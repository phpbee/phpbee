...
jQuery.fn.rte = function(options, editors) {
	...
	$(this).each(function(i) {
		...
		//alert (this.getAttribute('_images'));
...
var lwRTE = function (textarea, options) {
	...
	this.textarea	= null;
	if(textarea.getAttribute('_images')) {
		this.images_link= textarea.getAttribute('_images');
		if (textarea.getAttribute('_files')) {
			this.files_link= textarea.getAttribute('_files');
		}
	}
	this.event		= null;
	this.range		= null;
...
lwRTE.prototype.selection_insert = function(html) {
	var rng	= this.get_selection_range();
	var iframe_window = this.iframe.contentWindow;

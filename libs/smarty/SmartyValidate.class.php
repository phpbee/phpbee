...
class SmartyValidate {
    ...
    function connect(&$smarty, $reset = false) {
	    ...
	    //mydump($url);
	    //if (!$reset) $reset=$url[path]!=$_SERVER[REQUEST_URI];
    ...
    function register_form($form, $reset = false) {
        ...
        if(SmartyValidate::is_registered_form($form) && !$reset && isset($_SESSION['SmartyValidate'][$form]) && $_SERVER[REQUEST_URI]==$_SESSION['SmartyValidate'][$form]['path']) {
            return false;
        } else {
	    ...
	    $_SESSION['SmartyValidate'][$form]['path']=$_SERVER[REQUEST_URI];
	...
	*/
        ...
        foreach($_sess as $_key => $_val) {
            ...
            if(!isset($formvars[$_field])
                ...
                if (empty($_val[ifField])) $_ret = !$_ret ? false : $_empty;
            } else {
		...
		if (!empty($_val[ifField])) {
			$_smarty_obj =& SmartyValidate::_object_instance('Smarty', $_dummy);
			 if (!$_smarty_obj->getVar($_val[ifField])) {
				 $_sess[$_key]['valid'] = 1;
			 }
		}

            if(!$_sess[$_key]['valid']) {
			//mydump("failed: $_field");
			//$_smarty_obj =& SmartyValidate::_object_instance('Smarty', $_dummy);
			//mydump($_smarty_obj);
    ...
    function _smarty_assign($vars = array()) {
        //mydump($_SESSION['SmartyValidate']) ;
        $_smarty_obj =& SmartyValidate::_object_instance('Smarty', $_dummy);
